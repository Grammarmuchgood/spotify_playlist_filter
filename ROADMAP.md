...
**Real-world query testing surfaced two structural gaps**, not bugs: (1) the pipeline filters out bad matches well but under-prioritizes genre — e.g. a "chill rap" query doesn't reliably put rap tracks at the top, since genre is currently just soft context baked into the LLM's prose, never a real ranking signal; (2) no artist-specific or "songs like X" query support, since artist identity was deliberately excluded from the embedded text. Artist-priority and "songs similar to X" scoped as separate, cheaper follow-on features, not built yet.

**Genre-aware hybrid ranking — built and tested**: `pipeline/genre_buckets.py` assigns every song to one of 17 canonical genre buckets (Hip-Hop/Rap, Pop, Rock, etc. — a controlled vocabulary, matched via embedding similarity, not a hand-built keyword dictionary) using real genre data when available, falling back to the song's own description text for the ~32 songs with none. `search/hybrid.py` fuses a genre-relevance ranking with the vibe ranking via **Reciprocal Rank Fusion** (combines by rank position, not raw score — no arbitrary weight to calibrate between two differently-scaled signals), then reranks the top-50 shortlist with a **`Qwen3-Reranker-0.6B`** cross-encoder for final precision.
Comparison test on "high energy rock songs": pure vibe-only-then-rerank produced ~30-45% genuinely-rock results in the top 20; adding the genre+RRF stage before reranking raised that to **85% (17/20)** — confirmed the genre layer does real, necessary work, not redundant complexity next to reranking.
Found (and partially fixed) two further bugs while stress-testing: (1) short, common-English-word genre tags ("jerk," "trap," "grime," "drill") were embedding-mismatched to unrelated buckets (e.g. "jerk" → Jazz) since an isolated ambiguous word has no context to disambiguate its niche musical sense — fixed by adding those specific terms as their own buckets, since a term matching *itself* is a far more reliable comparison than bridging to an unrelated bucket name (confirmed: "trap" went from Rock 0.581 to Trap 0.932). Confirmed via direct testing that bucket count doesn't meaningfully affect runtime even at 100+ buckets (~0.03ms per query-vs-bucket comparison) — the real cost lives in the reranker's per-candidate inference, not bucket lookup. (2) MusicBrainz artist search can resolve to the wrong same-named artist (found: "Cochise" matched a krautrock group over the correct hip-hop producer, by 1 point) — not yet fixed, same bug class as the iTunes matcher fix, needs the same "clear margin over the runner-up" validation applied to the MusicBrainz artist-search step.
**Considered and explicitly rejected**: an LLM-based multi-category reranker (explicit artist/genre/energy/vibe judgments per candidate) — more reliable in principle, but costs a real per-query LLM call per shortlist candidate, breaking the "instant, free, local" property the whole local-embedding search was built for. A confidence-margin-triggered dynamic bucket-creation system was discussed as a more general, automatic version of the manual "add Trap/Drill/Grime/Jerk" fix, but not built — real value for an ongoing service, not clearly justified for a one-time batch against a fixed playlist snapshot.

**Found and fixed a real data-integrity bug while discussing what happens if the source playlist changes**: `fetch_playlist.py` used `INSERT OR REPLACE`, which (confirmed via direct testing of SQLite's actual behavior) deletes the whole existing row on a primary-key conflict and reinserts it using only the columns named in the statement — silently wiping `audio_features`/`lyrics`/`description`/`embedding`/`genre_bucket` back to `NULL` for *every* track, not just changed ones, on every re-fetch. Fixed with a proper upsert (`ON CONFLICT DO UPDATE`) that only touches the metadata columns this function owns. Verified live against the real playlist, which had genuinely grown by 2 tracks during this session: all 647 already-processed songs kept their full computed data, the 2 new tracks landed with `NULL` downstream fields, and every other pipeline stage's existing "only process what's still NULL" logic picked them up automatically — ran the full pipeline for both, no issues.
